...
while ($row = $result->fetch_assoc()) {
    ...
    $expired = checkExpired($conn,$row['product_id'],$row['stock_id']);
    $date =date('Y-m-d');
    if($expired < $date){
        $exp = "<span class='badge badge-sm expired'><i class='fa fa-ban'></i>(expired)</span>";
    }else{
        $exp = "";
    }
    echo "  <tr>
                <td >" . $row["product"] ." ".$status." ".$exp. "</td>
                <td>" . $row["quantity"] . "</td>
                <td>" . $row["remaining"] . "</td>
                <td>" . $row["datein"] . "</td>
