...
function deleteproductlist($conn, $id)
{
    ...
    $stmt->execute();
    echo "<script>alert('Deleted Successfully');";
    echo "setTimeout(function(){window.location = 'index.php?title=product';}, 100);</script>";
...
function createStock($conn, $category, $quantity, $expiration_date)
{
    $date = date('Y-m-d');
    //prepare query statement
    $stmt = $conn->prepare("INSERT INTO stock (product_id,quantity,remaining,expiration_date,datein) VALUES(?,?,?,?,?)");
    //bind parameters
    $stmt->bind_param("siiss", $category, $quantity, $quantity, $expiration_date, $date);
    //execute query
    $stmt->execute();
...
{
    $stmt = $conn->prepare("SELECT `remaining`,stock_id FROM stock WHERE (product_id= ? AND expiration_date > CURRENT_DATE) AND remaining > 0 ORDER BY expiration_date ASC LIMIT 1");
...
// Check ExpiredProductBatches Function <Start>
function checkExpired($conn, $category, $stockId): string
{
    $stmt = $conn->prepare("SELECT expiration_date FROM stock WHERE product_id= ? AND stock_id=?");
...
// Check ExpiredProductBatches Function <End>
// Check Stock Function <Start>

function checkStock($conn, $category)
{
    $stmt = $conn->prepare("SELECT SUM(`remaining`) AS Total_stock_remaining FROM stock WHERE product_id= ? AND expiration_date > CURRENT_DATE");
    $stmt->bind_param('i', $category);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
